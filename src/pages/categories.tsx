                ...
                <TableRow>
                  <TableHead>
                    <Button variant="ghost" onClick={() => handleSort("CategoryId")}>
                      <span className="flex items-center"> {/* Single child for Button */}
                        <span>ID</span> {/* Wrap text in span */}
                  ...
                  <TableHead>
                    <Button variant="ghost" onClick={() => handleSort("CategoryName")}>
                      <span className="flex items-center"> {/* Single child for Button */}
                        <span>Name</span> {/* Wrap text in span */}
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </span>
                    </Button>
                  </TableHead>
                  {/* <TableHead> // Removed created_at column
              ...
              <TableBody>
                {loading ? (
                  [...Array(pageSize)].map((_, i) => (
                    <TableRow key={i}>
                      <TableCell colSpan={3}><Skeleton className="h-8 w-full" /></TableCell> {/* Adjusted colspan */}
                    </TableRow>
                  ))
                ) : categories.length > 0 ? (
                  categories.map((category) => (
                    <TableRow key={category.CategoryId}>
                      ...
                      {/* <TableCell>{new Date(category.created_at).toLocaleDateString()}</TableCell> // Removed created_at display */}
